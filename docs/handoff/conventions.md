...
| C-003 | Adding a CLI entry point | Add it to `[project.scripts]` in `pyproject.toml` and document it per the `cli-documentation` standard (`docs/usage.md`). |
| C-004 | Editing `.standards/config.toml` frontmatter scope | Keep enforcement on durable docs and preserve declared operational, harness, generated, and fixture exclusions. |
...
**Applies when:** working in `integrations/agents/`, `editors/vscode/`, `editors/kate/`, or the language server.
...
**Rule:** Follow the `cli-documentation` standard's section registry (`NAME`, `SYNOPSIS`, `DESCRIPTION`, `OPTIONS`, `EXIT STATUS`, `ENVIRONMENT`, `FILES`, `EXAMPLES`, `NOTES`, `SEE ALSO`) in `docs/usage.md` or a dedicated per-command page.
...
**Applies when:** editing `.standards/config.toml` frontmatter scope or adopting frontmatter elsewhere in the repo.

**Rule:** Preserve the configured `docs/**/*.md`, `editors/**/*.md`, and `integrations/agents/**/*.md` durable-document corpus and its explicit exclusions.
...
**Why:** ADR-0004 makes the integration relationship durable while keeping review and approval independent of the local mechanical safeguards. `scripts/policy/install-hooks.sh` installs the safeguards and preserves predecessor hooks.
